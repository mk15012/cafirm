...
export async function getRootCAId(userId: number): Promise<number | null> {
  try {
    ...
    const visited = new Set<number>([userId]); // Prevent infinite loops
    ...
    while (currentUserId && !visited.has(currentUserId)) {
      ...
      currentUserId = currentUser.reportsToId || 0;
    }
 ...
 */
export async function getCAOrganizationUserIds(caId: number): Promise<number[]> {
  try {
    // Get all users who report directly or indirectly to this CA
    const getAllTeamMembers = async (managerId: number, collected: Set<number> = new Set()): Promise<number[]> => {
